...
function calculatePayouts(entries: EntryLike[]): {
  rows: PayoutRow[];
  dqRefundList: EntryLike[];
  unaccounted: number;
} {
  // DQ refunds only go to entries that actually paid
  const dqRefundList = entries
    .filter((e) => e.disqualified && e.paidAt !== null)
  ...
  if (scored.length === 0) {
    return { rows: [], dqRefundList, unaccounted: 0 };
  }
  ...
  const unaccounted = prizePool - totalPaid + (40 * dqRefundList.length);

  return { rows, dqRefundList, unaccounted };
}
...
export default async function FinancesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { rows, dqRefundList } = scoresAvailable
    ? calculatePayouts(allEntries)
    : { rows: [], dqRefundList: [] };
        ...
        {scoresAvailable && (
          <div className="rounded-xl border border-gray-200 shadow-md overflow-hidden bg-white">
            <div className="bg-red-700 px-5 py-3">
              <h3 className="font-bold text-white text-sm tracking-wide uppercase">DQ&apos;d Entries — Entry Returned</h3>
            </div>
            {dqRefundList.length === 0 ? (
              <p className="px-5 py-4 text-sm text-gray-400">No DQ&apos;d entries with a paid entry fee.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-100 border-b-2 border-gray-300 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                    <th className="text-left px-5 py-3">Entry</th>
                    <th className="text-right px-5 py-3">Refund</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {dqRefundList.map((entry, i) => (
                    <tr key={entry.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      <td className="px-5 py-3">
                        <div className="font-medium text-gray-800">{entry.entryName}</div>
                        {entry.user?.name && (
                      ...
                      <td className="px-5 py-3 text-right font-bold text-green-700">$40</td>
                    </tr>
                  ))}
                </tbody>
              </table>
